...
const ReentriesAlertsTable = async () => {
  ...
  return (
    <ReentriesAlertsDataTable data={data} haveAccessToAlerts={isAgencyUser(session?.user.role) || isGovUser(session?.user.role) || isInternationalUser(session?.user.role)} />
  );
};
