...
const ScheduleBannerConfirm = ({ title, banner }: ScheduleBannerConfirmProps) => {
  ...
  return (
    <>
      {isSuccess && (
        <TopNotificationBanner status="success" heading={t('Success_banner.title', { title })} aria-label="Set up incident banner">
      ...
      {confirmModal && (
        <TopNotificationBanner status="error" heading={t('Confirm_banner.title', { title })} aria-label="Display confirmation banner">
          <p className="govuk-body">
